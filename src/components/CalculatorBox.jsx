...
const factorialOperation = (nb) => {
  for (let index = factorial.length; index <= nb; index++) {
    factorial[index] = factorial[index - 1] * index;
    if (!isFinite(factorial[index])) {
...
const separatorHandler = (string) => {
  ...
  while (index < len) {
    ...
    if (
      ...
      return { separatorError: "Error: Wrong Operation" };
    }
  }
  if (number !== "") {
    let myNumber = Number(number);
    if (!isNumber(myNumber)) {
      return { separatorError: "Error: Not a Number" };
    }
    stack.push(myNumber);
    number = "";
  }
...
const parenthesisHandler = (formula) => {
  let nbopen = 0,
    nbclose = 0;
  for (let char of formula) {
    if (char === "(") {
      ...
      nbclose++;
      if (nbclose > nbopen) {
        return { parenthesisError: "Error : Wrong Parenthesis" };
      }
    }
  }
...
const calculatorHandler = (formula, start, end, angle) => {
  let index = start,
    stack = [];
  ...
  while (index <= end) {
    ...
    if (elt === "(") {
      ...
      let { result, calculatorError } = calculatorHandler(
        formula,
        index + 1,
        indexClose - 1,
        angle
      ...
      if (calculatorError) {
        return { calculatorError: calculatorError };
      }
      stack.push(result);
      index = indexClose + 1;
    } else if (elt === ")") {
      return { calculatorError: "Error: Wrong Parenthesis Calculator" };
    } else {
      stack.push(elt);
      index++;
    }
    ...
    if (isNumber(current)) {
      while (
        ...
        if (stack2[stack2.length - 1] === "^") {
          ...
          current = operationHandler(stack2.pop(), current, angle);
        }
      }
    }
    stack2.push(current);
    ...
    if (isNumber(current)) {
      while (
        ...
        if (isNumber(stack2[stack2.length - 1])) {
          current = operationHandler("x", stack2.pop(), current, angle);
          ...
          return {
            calculatorError: "Error: Wrong Times / Division Calculator",
          };
        }
      }
    }
  ...
  while (index < stack2.length) {
    result = operationHandler(stack2[index], stack2[index + 1], result, angle);
    index += 2;
  }
...
const CalculatorBox = () => {
  ...
  const formulaHandler = (operation) => {
    if (operation === "rad" || operation === "deg") {
      ...
      let { stack, separatorError } = separatorHandler(memory.formula);
      if (separatorError) {
        setMemory({ ...memory, ans: separatorError, formula: "" });
        return;
      }
      let { formula, parenthesisError } = parenthesisHandler(stack);
      if (parenthesisError) {
        setMemory({ ...memory, ans: parenthesisError, formula: "" });
        return;
      }
      let { result, calculatorError } = calculatorHandler(
        formula,
      ...
      if (calculatorError) {
        setMemory({ ...memory, ans: calculatorError, formula: "" });
        return;
      }
    ...
    return (
      <td key={key}>
        <button
          ...
          onClick={() => formulaHandler(item.operator ?? item.text)}
        >
          {item.text}
        </button>
      </td>
        ...
        <table>
          <tbody>
              ...
              <td colSpan="2">
                <button
                  className={`${styles.angle} ${
                    memory.angle === "rad" ? styles.on : styles.off
                  }`}
            ...
            <tr>
              {[
                { text: "Inv" },
                { text: "sin", operator: "sin(" },
                { text: "ln", operator: "ln(" },
                { text: "7", class: styles.main },
                { text: "8", class: styles.main },
                { text: "9", class: styles.main },
                { text: "÷", operator: "/" },
              ].map(BasicButton)}
            </tr>
            <tr>
              {[
                { text: "π" },
                { text: "cos", operator: "cos(" },
                { text: "log", operator: "log(" },
                { text: "4", class: styles.main },
                { text: "5", class: styles.main },
                { text: "6", class: styles.main },
                { text: "x" },
              ].map(BasicButton)}
            </tr>
            <tr>
              {[
                { text: "e" },
                { text: "tan", operator: "tan(" },
                { text: "√", operator: "√(" },
                { text: "1", class: styles.main },
                { text: "2", class: styles.main },
                { text: "3", class: styles.main },
                { text: "-" },
              ].map(BasicButton)}
            </tr>
            <tr>
              {[
                ...
                { text: "!" },
                { text: "x^y", operator: "^(" },
                { text: "0", class: styles.main },
                { text: ".", class: styles.main },
                { text: "=", class: styles.equal },
                { text: "+" },
              ].map(BasicButton)}
            </tr>
          </tbody>
